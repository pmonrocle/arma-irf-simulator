...
st.set_page_config(layout="wide")
st.title("Simulador AR / MA / ARMA e IRF (con checks correctos)")
...
# -----------------------------
# Proceso (ARMA) y checks correctos
# -----------------------------
np.random.seed(int(seed))
...
# Checks nativos y correctos (evita el error de np.roots mal aplicado)
ar_ok = proc.isstationary
ma_ok = proc.isinvertible
...
with st.sidebar:
    ...
    st.subheader("Checks")
    if model_type in ["AR", "ARMA"]:
        st.write(f"Estacionariedad AR: {'✅' if ar_ok else '❌'}")
    if model_type in ["MA", "ARMA"]:
        st.write(f"Invertibilidad MA: {'✅' if ma_ok else '❌'}")

# Cortar si no cumple
if (model_type in ["AR", "ARMA"] and not ar_ok) or (model_type in ["MA", "ARMA"] and not ma_ok):
    st.error(
        "Parámetros no válidos: el AR no es estacionario y/o el MA no es invertible.\n\n"
        "Ajusta φ y/o θ para que el proceso sea estable (AR estacionario, MA invertible)."
    )
    st.stop()
